...
export function DashboardHeader() {
  const { profile, toggleMobileSidebar } = useAppStore();

  return (
    <header
      className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-navy-700/30 bg-charcoal-900/95 px-4 backdrop-blur-sm sm:px-6"
    >
      {/* Mobile Hamburger + Desktop margin handled via classes */}
      <div className="flex items-center gap-3 flex-1">
        <button
          onClick={toggleMobileSidebar}
          className="flex h-10 w-10 items-center justify-center rounded-lg text-ivory-600 transition-colors hover:bg-navy-800 hover:text-ivory-300 lg:hidden"
        >
